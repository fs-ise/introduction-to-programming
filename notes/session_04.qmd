...
- plan: download files into working directory? avoid working in this repo and avoid copying files during the session

- Before the pandas exercises, introduce the `data.zip` archive available in Canvas. Confirm that it contains `Bikesales.xlsx`, `Bikesales.csv`, `Exped.xlsx`, and `Sales.xlsx`, and that students have extracted it.
...
::: {.teaching-file}
[`../materials/session_04/Bikesales.xlsx`](../materials/session_04/Bikesales.xlsx){.teaching-button .teaching-button-exercise}
:::

::: {.callout-important title="Required datasets"}
Download `data.zip` from Canvas and extract it into your working directory. The archive contains `Bikesales.csv`, `Bikesales.xlsx`, `Exped.xlsx`, and `Sales.xlsx`. Make sure Spyder's working directory points to the folder containing these files before running the examples.
:::
...
**When working on Linux, show this hypothetically:**
...
The relative path is resolved against the active working directory. `Bikesales.csv` is included in the same Canvas `data.zip` archive.
...
::: {.teaching-file}
[`../materials/session_04/Exped.xlsx`](../materials/session_04/Exped.xlsx){.teaching-button .teaching-button-exercise}
:::

`Exped.xlsx` is included in the Canvas `data.zip` archive. `head()` previews the first rows; `index=False` avoids writing the DataFrame index as an extra CSV column.
