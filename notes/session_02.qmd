...
Solve together in [Excel_Exercises_in_class_session_1.xlsx](Excel_Exercises_in_class_session_1.xlsx): Sheet `Moving Average`
...
- Review the repayment condition introduced in Session 1.
- Ask why the fixed repayment amount cannot simply be copied through all 20 periods.
- Reiterate that the final repayment must not exceed the remaining debt.
- Briefly contrast the declining payments of a fixed-principal loan with the constant payments of an annuity loan.

Solve together in [Excel_Exercises_in_class_session_1.xlsx](Excel_Exercises_in_class_session_1.xlsx): Sheet `Installment`

![](../images/excel_installment_loan_1_solution.png){fig-align="center" width="90%"}

---

Solve together in [Excel_Exercises_in_class_session_1.xlsx](Excel_Exercises_in_class_session_1.xlsx): Sheet `Annuity`
...
If it was not covered in the previous session: complete slide 28
...
Solve together in [Excel_Exercises_in_class_session_1.xlsx](Excel_Exercises_in_class_session_1.xlsx): Sheet `Bar_Sales`

Task:

- Retrieve the unit price corresponding to each sold beverage.
- Calculate the total price from quantity and unit price.
- Only the beverage and quantity should be entered manually.

Suggested progression:

1. Identify the lookup value.
2. Select the price-list range.
3. Determine the column containing the price.
4. Use an exact lookup.
5. Copy the formula down.
6. Calculate the total price.

::: {.callout-tip title="Teaching point"}
Ask which references must remain fixed when the formula is copied and which references must change.
:::

![](../images/excel_bar_sales_solution.png){fig-align="center" width="90%"}
...
[Clients.xlsx](../materials/Clients.xlsx)

The client number is entered once. The following fields should update automatically:
...
- Use the status column as the lookup value.
- Use the two small mapping tables at the bottom of the sheet.
- Retrieve the gift and packaging independently.
- Copy both formulas down.

Encourage students to use `XLOOKUP`. A `VLOOKUP` solution can be accepted if the lookup tables are arranged appropriately.

::: {.callout-note title="Screenshot placeholder"}
Add the **Christmas gift** solution screenshot from `Solutions.pptx`.
:::

## Slide 36 — Course grading (take-home exercise)

The selected student ID should determine:
...
1. Retrieve the three test scores.
2. Retrieve or calculate the homework average.
3. Apply the corresponding weights.
4. Add the weighted components.
...
Solve together: [Bikesales.xlsx](../materials/Bikesales.xlsx)
...
[Sales_Table.xlsx](../materials/Sales_Table.xlsx)
